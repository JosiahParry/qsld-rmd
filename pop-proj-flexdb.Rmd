---
title: "Parameterized Flexdashboard"
output: 
...
plotly::ggplotly({age_pop %>% 
  ...
  labs(x = "",
       ...
       subtitle = glue::glue("{params$region}"),
       color = "Series")})
```
...
plotly::ggplotly({ggplot(prop_pop, aes(projection_year, perc_pop, color = age_bucket)) +
  geom_point() +
  geom_line() + 
  ...
  labs(y = "% of population", x = "",
       title = "Change in population for youth and seniors",
       color = "")}) 
```
...
age_pop %>% 
  #filter(age_bucket != "15-64") %>% 
  filter(projection_year %in% c("2016", "2041")) %>% 
  group_by(series, projection_year, age_group) %>% 
  summarise(pop = sum(value)) %>% 
  mutate(perc_pop = pop / sum(pop)) %>% 
  ungroup() %>% 
  mutate_if(is.double, ~round(.*100, 2)) %>% 
